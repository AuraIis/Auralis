...
for MODEL in \
  ...
  echo "##### $MODEL #####"
  python scripts/eval/benchmark_mc.py --model "$MODEL" --tasks mmlu_de,arc_de,hellaswag_de --limit 300 2>&1 | grep -E "RESULT|Error|Traceback|FEHLER"
done
echo "ALL_BENCH_DE_DONE"
