...
def main() -> None:
    ...
    def rmsnorm():
        h = x(T, D_MODEL)
        w = (torch.randn(D_MODEL, device=dev, dtype=torch.float32)).requires_grad_(True)
    ...
    def ffn():  # SwiGLU: gate, up, silu-mul, down
        h = x(T, D_MODEL)
        wg = lin(D_MODEL, D_FFN)
        wu = lin(D_MODEL, D_FFN)
        wd = lin(D_FFN, D_MODEL)
    ...
    def rope():
        B = max(1, T // args.seq)
        S = args.seq
        q = x(B, N_HEADS, S, HEAD_DIM)
        k = x(B, N_HEADS, S, HEAD_DIM)
