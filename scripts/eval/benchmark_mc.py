...
class HelixLM:
    def __init__(
        self,
        ckpt,
        cfg=str(REPO / "configs/model/helix_v2_1b.yaml"),
        tok=str(REPO / "tokenizer/helix_v2_tokenizer.model"),
    ):
...
def load_task(name, limit):
    ...
    if name == "mmlu":
        ...
        for r in ds:
            q = r["question"]
            ch = r["choices"]
            ans = r["answer"]  # int 0-3
            ctx = f"{q.strip()}\n" + "".join(f"{l}. {c}\n" for l, c in zip("ABCD", ch)) + "Answer:"
            items.append(
                {"ctx": ctx, "conts": [f" {l}" for l in "ABCD"], "gold": ans, "norm": False}
            )
        ...
        for r in ds:
            ...
            items.append(
                {"ctx": ctx, "conts": [f" {t}" for t in texts], "gold": gold, "norm": True}
            )
    elif name == "hellaswag":
        ...
        for r in ds:
            ...
            items.append(
                {
                    "ctx": ctx,
                    "conts": [" " + e.strip() for e in r["endings"]],
                    "gold": int(r["label"]),
                    "norm": True,
                }
            )
        ...
        items = []
        for r in ds:
            if r["answer"] not in "ABCD":
                continue
            opts = [r["option_a"], r["option_b"], r["option_c"], r["option_d"]]
            gold = "ABCD".index(r["answer"])
            ctx = (
                f"{r['instruction'].strip()}\n"
                + "".join(f"{l}. {c}\n" for l, c in zip("ABCD", opts))
                + "Antwort:"
            )
            items.append(
                {"ctx": ctx, "conts": [f" {l}" for l in "ABCD"], "gold": gold, "norm": False}
            )
    elif name == "arc_de":
