...
def main() -> int:
    ...
    ap.add_argument("--input", action="append", required=True, help="lang=path (repeatable)")
    ap.add_argument(
        "--loss", action="append", default=[], help="lang=per_token_nats (repeatable; optional)"
    )
    ...
    print(
        f"tokenizer vocab: {sp.get_piece_size()}  | sample: {args.sample_bytes / 1e6:.1f} MB/lang\n"
    )
    ...
    for lang, path in inputs.items():
        ...
        row = {
            ...
            "tokens": ntok,
            "tokens_per_byte": tpb,
            "tokens_per_word": ntok / max(1, nwords),
    ...
    if len(scored) >= 2:
        ...
        print(
            f"fertility ratio ({b['lang']}/{a['lang']} tokens/byte): "
            f"{b['tokens_per_byte'] / a['tokens_per_byte']:.3f}"
        )
        print(
            f"per-token loss ratio ({b['lang']}/{a['lang']}):        "
            f"{b['loss_tok_nats'] / a['loss_tok_nats']:.3f}"
        )
        print(
            f"bits-per-byte: {a['lang']}={a['bits_per_byte']:.3f}  "
            f"{b['lang']}={b['bits_per_byte']:.3f}"
        )
        print(
            f"TRUE gap ({b['lang']}/{a['lang']} bpb): "
