...
def ppl_on(model, sp, path, dev, max_len=1024, max_docs=200):
...
def main():
    ...
    for pair in a.sets.split(","):
        ...
        avg, ppl, nd, tk = ppl_on(model, sp, path, dev, max_docs=a.max_docs)
        print(f"  {name:8} loss={avg:.3f} ppl={ppl:7.1f}  (docs {nd}, tokens {tk})", flush=True)
