        ...
        def make_wrapped(orig, idx, ltype):
            def wrapped(x, rope=None):
                ...
                try:
                    ...
                    print(
                        f"  L{idx:02d} {ltype:18s} dt={elapsed_ms:8.1f}ms "
                        f"mem={_gb(mem_after):6.2f}GB d={delta:+.3f}GB "
                        f"peak={_gb(mem_peak):6.2f}GB",
                        flush=True,
                    )
...
def main():
    ...
    ap.add_argument(
        "--batch-size", type=int, default=None, help="override training.batch_size_per_device"
    )
    ap.add_argument(
        "--no-grad-ckpt", action="store_true", help="disable gradient checkpointing for this run"
    )
    args = ap.parse_args()

    print("=== diagnose_layer_memory ===", flush=True)
    ...
    for step in range(args.steps):
        ...
        try:
            ...
            torch.cuda.synchronize()
            print(
                f"  fwd done in {(time.time() - t0) * 1000:.0f}ms loss={loss.item():.3f} "
                f"mem={_gb(torch.cuda.memory_allocated()):.2f}GB",
                flush=True,
            )

            print("  >>> backward starting", flush=True)
            tb0 = time.time()
            loss.backward()
            torch.cuda.synchronize()
            print(
                f"  bwd done in {(time.time() - tb0) * 1000:.0f}ms "
                f"mem={_gb(torch.cuda.memory_allocated()):.2f}GB "
                f"peak={_gb(torch.cuda.max_memory_allocated()):.2f}GB",
                flush=True,
            )
            optimizer.step()
            optimizer.zero_grad()
