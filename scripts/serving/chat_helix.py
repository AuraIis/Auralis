...
MAP = {
    "corrective": "sft_corrective_v3",
    "instruct": "sft_corrective_v3",
    "grounded": "sft_grounded_v4",
    "code": "sft_code_v3",
}
...
MAXNEW = int(os.environ.get("MAXNEW", "220"))
...
if not os.path.exists(ADP):
    print("!! Adapter nicht gefunden. Verfuegbar:")
    os.system("ls /workspace/v2data/checkpoints/ | grep -E 'sft_|code_'")
    sys.exit(1)
    ...
    with torch.no_grad():
        for _ in range(MAXNEW):
            ...
            if TEMP > 0:
                ...
                p = torch.softmax(v, -1)
                nid = int(ix[torch.multinomial(p, 1)])
            else:
