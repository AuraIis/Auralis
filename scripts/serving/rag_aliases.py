...
import os, sqlite3, urllib.request, time
RAG = "/workspace/v2data/rag"; DB = f"{RAG}/dewiki.fts5.db"
PAGE = f"{RAG}/dewiki-latest-page.sql.gz"; REDIR = f"{RAG}/dewiki-latest-redirect.sql.gz"
BASE = "https://dumps.wikimedia.org/dewiki/latest/"
def dl(name, path):
    if os.path.exists(path) and os.path.getsize(path) > 1_000_000:
        print(f"  have {name} ({os.path.getsize(path)//1024//1024} MB)", flush=True); return
    print(f"  downloading {name} ...", flush=True); urllib.request.urlretrieve(BASE + name, path)
    print(f"  -> {os.path.getsize(path)//1024//1024} MB", flush=True)
dl("dewiki-latest-redirect.sql.gz", REDIR); dl("dewiki-latest-page.sql.gz", PAGE)
from mwsql import Dump
t0 = time.time()
# 1) page_id -> title (underscores->spaces) for namespace-0 redirect pages
dp = Dump.from_file(PAGE); cn = list(dp.dtypes.keys())
