...
def main() -> int:
    ...
    p.add_argument(
        "--threshold", type=int, default=3, help="keep docs with score >= threshold (default 3)"
    )
    ...
    p.add_argument("--batch-size", type=int, default=8, help="distilabel parallelism per LLM call")
    p.add_argument(
        "--chunk-size",
        type=int,
        default=500,
    ...
    if not api_key:
        ...
        if "openrouter.ai" not in args.base_url:
            ...
            print(f"  using dummy key for non-OpenRouter endpoint {args.base_url}", flush=True)
        else:
            sys.exit(
                f"FATAL: {args.api_key_env} env var is required for OpenRouter ({args.base_url})"
            )
    ...
    if n_too_short or n_too_long:
        print(
            f"  pre-filtered: {n_too_short} too-short (<{args.min_chars} chars), "
            f"{n_too_long} too-long (>{args.max_chars} chars)",
            flush=True,
        )
    if n_resumed:
    ...
    print()
    print("=== Ask-LLM (DeepSeek) results ===")
