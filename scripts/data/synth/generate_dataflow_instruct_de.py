...
MATH_TEMPLATES = [
    ...
    (
        "In einer Klasse mit {n} Schülern sind {pct}% Mädchen. Wie viele Jungen sind in der Klasse?",
        None,
    ),
    (
        "Ein Auto verbraucht im Schnitt {v} Liter pro 100 km. Wie viel Benzin braucht es für {d} km?",
        None,
    ),
    (
        "Ein Kapital von {k}€ wird mit {r}% jährlich verzinst. Wie viel Zinsen nach {y} Jahren (einfache Verzinsung)?",
        None,
    ),
    (
        "Eine Wand ist {w} m breit und {h_wall} m hoch. Eine Tapetenrolle deckt {f} m². Wie viele Rollen (aufrunden)?",
        None,
    ),
    (
    ...
    (
        "Auf einer Karte 1:{scale} sind zwei Städte {cm} cm entfernt. Wie weit in Wirklichkeit?",
        None,
    ),
    (
        "Ein Tank fasst {tank_v} Liter und ist {tank_p}% gefüllt. Pro Stunde laufen {l} Liter aus. Wann leer?",
        None,
    ),
    ...
    (
        "Eine Maschine produziert {n_parts} Teile/h. Wie viele in 8h-Schicht? Und in Woche bei 5 Schichten?",
        None,
    ),
    ("Ein Hotel hat {rooms} Zimmer, {dbl} Doppel, Rest Einzel. Anteil Einzelzimmer in %?", None),
    (
        "Eine Druckerei: {grundgebuehr}€ Grundgebühr + {ct}ct/Seite. Kosten für {pages} Seiten?",
        None,
    ),
    ...
    (
        "Pool: Zufluss füllt in {pool_fill}h, Abfluss leert in {pool_drain}h. Wie lang bei beiden?",
        None,
    ),
    (
        "Gemüsehändler: {kg_apfel} kg Äpfel zu {pr_apfel}€/kg, {kg_birne} kg Birnen zu {pr_birne}€/kg. Gesamt?",
        None,
    ),
    (
        "Bahnstrecke {strecke} km. Zug A fährt mit {va} km/h, Zug B mit {vb} km/h. Entgegengesetzt. Wann treffen sie sich?",
        None,
    ),
    (
        "Opa ist {opa_alter} Jahre alt, Enkel {enkel_alter}. In wie vielen Jahren ist Opa genau 3x so alt wie Enkel?",
        None,
    ),
    (
        "Ein Schwimmbecken ist {pool_l} m lang, {pool_w} m breit, {pool_d} m tief. Wie viele Liter Wasser passen rein?",
        None,
    ),
    (
        "Ein Fahrrad kostet {bike_price}€. Rabatt {rabatt}%. Wie viel spart man und wie viel zahlt man?",
        None,
    ),
    ...
    (
        "Dreieck: Seiten {tri_a} cm, {tri_b} cm, {tri_c} cm. Umfang und (falls rechtwinklig) Fläche?",
        None,
    ),
    (
        "Bus fährt um {bus_start} Uhr los, kommt um {bus_end} Uhr an. Strecke {bus_km} km. Durchschnittsgeschwindigkeit?",
        None,
    ),
    (
        "Sparvertrag: monatlich {spar_mon}€ einzahlen, {spar_monate} Monate, {spar_zins}% p.a. Endkapital (einfach)?",
        None,
    ),
    ...
    (
        "Durchschnittsberechnung: Noten {note1}, {note2}, {note3}, {note4}, {note5}. Durchschnitt?",
        None,
    ),
    (
        "Wahrscheinlichkeit: Urne mit {urne_r} roten und {urne_b} blauen Kugeln. 2 ziehen ohne Zurücklegen. P(beide rot)?",
        None,
    ),
]
...
CONCEPTS_DE = [
    ...
    "Epigenetik",
    "Mikrobiom",
    "Immunsystem (angeboren vs erworben)",
    "Viren vs Bakterien",
    "Stammzellen",
    "Neuroplastizität",
    # Physik
    ...
    "Laser",
    "Tunneleffekt",
    "Schrödinger-Gleichung (vereinfacht)",
    # Chemie
    ...
    "Marathon (Geschichte + Physiologie)",
    "Olympische Spiele (Antike vs Modern)",
    # Psychologie
    "Kognitive Dissonanz",
    ...
    "Cholesterin (HDL/LDL)",
    "Allergie vs Autoimmunerkrankung",
    # Musik
    "Dur vs Moll",
    "Synkope (Musik)",
    "Kontrapunkt",
...
TRANSLATION_TASKS = [
    ...
    (
        "DE→EN",
        "Das föderale System Deutschlands verteilt Gesetzgebungskompetenzen auf Bund und Länder.",
    ),
    ...
    (
        "DE→EN",
        "Der Treibhauseffekt beschreibt die Erwärmung der Erdatmosphäre durch bestimmte Gase.",
    ),
    ...
    (
        "DE→EN",
        "Maschinelles Lernen extrahiert Muster aus Daten ohne explizit programmiert zu werden.",
    ),
    ...
    (
        "EN→DE",
        "Knowledge distillation transfers a teacher model's behavior into a smaller student.",
    ),
    ...
    (
        "EN→DE",
        "The federal system distributes legislative powers between the federation and the states.",
    ),
    ...
    (
        "EN→DE",
        "Democracy requires the separation of powers into legislative, executive, and judicial branches.",
    ),
]
...
SMALLTALK_GREETINGS = [
    ...
    "Guten Abend! Schön dich zu treffen.",
...
SMALLTALK_HOWRU = [
    ...
    "Geht's dir gut?",
    "Wie ist dein Tag so?",
    "Na, wie läuft der Tag?",
    "Wie fühlst du dich heute?",
]

SMALLTALK_CASUAL = [
    "Langweilig heute, oder?",
...
SMALLTALK_ABOUT_AI = [
    ...
    "Hast du einen Namen?",
    "Was kannst du alles?",
    "Bist du besser als ChatGPT?",
    "Träumst du?",
    "Was ist deine Lieblingsfarbe?",
    "Magst du Menschen?",
    "Hast du Freunde?",
    "Bist du manchmal traurig?",
    "Was machst du wenn niemand mit dir redet?",
    "Wie lernst du neue Dinge?",
