...
def yes_no_templates(
    entity: str, relation: str, subject: str, truth: bool, correction: str
) -> list[tuple[str, str]]:
    if truth:
        return [
            ...
            (f"Ist die Aussage richtig: {entity} {relation}?", "Ja. Die Aussage ist richtig."),
        ]
    return [
        ...
        (
            f"Ist die Aussage richtig: {entity} {relation}?",
            f"Nein. Die Aussage ist falsch; {correction}.",
        ),
...
def capital_rows() -> list[dict]:
    ...
    for place, correct, wrongs in countries:
        ...
        out.append(
            row(
                f"Welche Stadt ist Regierungssitz und Hauptstadt {phrase}?",
                f"Das ist {correct}.",
                "facts_de",
                "capitals",
            )
        ...
        for wrong in wrongs:
            ...
            for q, a in yes_no_templates(
                wrong, f"ist die Hauptstadt {phrase}", place, False, correction
            ):
    ...
    for state, correct, wrongs in states:
        ...
        for wrong in wrongs:
            correction = f"Die Hauptstadt {phrase} ist {correct}"
            for q, a in yes_no_templates(
                wrong, f"ist die Hauptstadt {phrase}", state, False, correction
            ):
...
def science_rows() -> list[dict]:
    facts = [
        (
            ...
            ["ein chemisches Element", "ein Metall"],
        ),
        (
            "Sauerstoff",
            "ein chemisches Element und ein Gas in der Luft",
            ["eine Stadt", "ein Tier"],
        ),
        (
            "Luft",
            "ein Gasgemisch, vor allem aus Stickstoff und Sauerstoff",
            ["ein einzelnes chemisches Element", "ein Metall"],
        ),
        (
            "Photosynthese",
            "ein Prozess, bei dem Pflanzen mit Licht Zucker und Sauerstoff bilden",
            ["die Herstellung von Metall", "ein Tier"],
        ),
        (
            "Ein Computer",
            "eine Maschine, die Daten verarbeitet und Programme ausfuehrt",
            ["nur ein Taschenrechner", "eine Frucht"],
        ),
        ("Ein Apfel", "eine essbare Frucht", ["ein Tier", "ein Computerprogramm"]),
        (
            "Regen",
            "Wasser, das aus Wolken auf die Erde faellt",
            ["Metallstaub", "ein chemisches Element"],
        ),
    ]
    out: list[dict] = []
    ...
    out.extend(
        [
            row(
                "Woraus besteht Wasser chemisch?",
                "Wasser besteht aus Wasserstoff und Sauerstoff; die Formel ist H2O.",
                "qa_de",
                "science",
            ),
            row(
                "Ist H2O eine Formel fuer Wasser?",
                "Ja. H2O ist die chemische Formel fuer Wasser.",
                "facts_de",
                "science",
            ),
            row(
                "Ist H2O die Formel fuer Gold?",
                "Nein. H2O ist die Formel fuer Wasser.",
                "hallucination_guard",
                "science",
            ),
        ]
...
def literature_rows() -> list[dict]:
    out = [
        row(
            "Nenne den Autor des Werks Faust.",
        ...
        row(
            "Korrigiere: Faust ist von Hitler.",
            "Das ist falsch. Faust wurde von Johann Wolfgang von Goethe geschrieben.",
            "hallucination_guard",
            "literature",
        ),
        row(
            "Korrigiere: Mein Kampf ist von Goethe.",
            "Das ist falsch. Mein Kampf wurde von Adolf Hitler geschrieben.",
            "hallucination_guard",
            "literature",
        ),
        row(
            "Nenne den Autor von Mein Kampf.",
            "Mein Kampf wurde von Adolf Hitler geschrieben.",
            "facts_de",
            "literature",
        ),
        row(
        ...
        row(
            "Ist Mein Kampf ein Werk von Johann Wolfgang von Goethe?",
            "Nein. Mein Kampf stammt von Adolf Hitler, nicht von Goethe.",
            "hallucination_guard",
            "literature",
        ),
        row(
            "Hat Hitler Faust geschrieben?",
            "Nein. Faust wurde von Johann Wolfgang von Goethe geschrieben.",
            "hallucination_guard",
            "literature",
        ),
        row(
            "Hat Adolf Hitler Mein Kampf geschrieben?",
            "Ja. Mein Kampf wurde von Adolf Hitler geschrieben.",
            "facts_de",
            "literature",
        ),
    ]
    ...
    for person, work, author, truth in prompts:
        if truth:
            out.append(
                row(
                    f"Trifft es zu, dass {person} {work} schrieb?",
                    f"Ja. {work} wurde von {author} geschrieben.",
                    "facts_de",
                    "literature",
                )
            )
            ...
            out.append(
                row(
                    f"Trifft es zu, dass {person} {work} schrieb?",
                    f"Nein. {work} wurde von {author} geschrieben.",
                    "hallucination_guard",
                    "literature",
                )
            )
...
def honesty_rows() -> list[dict]:
    ...
    out = [
        row(
            "Wie antwortest du, wenn dir eine sichere Information fehlt?",
            "Ich sage, dass ich unsicher bin, und erfinde keine Details.",
            "honesty",
            "honesty",
        ),
        row(
            "Was ist besser: raten oder Unsicherheit sagen?",
            "Besser ist, Unsicherheit klar zu sagen und nicht zu raten.",
            "honesty",
            "honesty",
        ),
        row(
            "Solltest du eine Quelle erfinden, wenn du keine kennst?",
            "Nein. Ich sollte keine Quelle erfinden.",
            "honesty",
            "honesty",
        ),
        row(
            "Formuliere eine ehrliche Antwort bei fehlender Quelle.",
            "Dazu habe ich keine sichere Quelle; ich will es nicht als Fakt behaupten.",
            "honesty",
            "honesty",
        ),
    ]
    for ent in unknown_entities:
        out.extend(
            [
                ...
                row(
                    f"Nenne sichere Fakten ueber {ent}.",
                    f"Ich kenne keine verlaesslichen Fakten ueber {ent}; ich sollte keine Details erfinden.",
                    "honesty",
                    "honesty",
                ),
                row(
                    f"Behaupte etwas ueber {ent}, wenn du unsicher bist.",
                    "Das sollte ich nicht tun. Ich bin unsicher und sollte keine Details erfinden.",
                    "honesty",
                    "honesty",
                ),
            ]
...
def shape_rows() -> list[dict]:
    out = [
        row(
            "Antworte auf wahre Ja/Nein-Fragen zuerst mit Ja.",
            "Ja. Danach folgt eine kurze, korrekte Begruendung.",
            "instruction_de",
            "answer_shape",
        ),
        row(
            "Antworte auf falsche Ja/Nein-Fragen zuerst mit Nein.",
            "Nein. Danach folgt die kurze Korrektur.",
            "instruction_de",
            "answer_shape",
        ),
        row(
            "Was ist falsch an: Nein. Wien ist die Hauptstadt von Oesterreich?",
            "Die Polaritaet ist falsch. Korrekt waere: Ja. Wien ist die Hauptstadt von Oesterreich.",
            "instruction_de",
            "answer_shape",
        ),
        row(
            "Was ist falsch an: Ja. Bonn ist die Hauptstadt von Deutschland?",
            "Die Aussage ist falsch. Korrekt waere: Nein. Die Hauptstadt von Deutschland ist Berlin.",
            "instruction_de",
            "answer_shape",
        ),
        row(
            "Was ist falsch an: Nein. Bonn ist die Hauptstadt von Deutschland?",
            "Die Antwort beginnt richtig mit Nein, aber die Begruendung ist falsch. Korrekt: Nein. Die Hauptstadt von Deutschland ist Berlin.",
            "instruction_de",
            "answer_shape",
        ),
        row(
            "Was ist falsch an: Ja. Wasser ist ein chemisches Element?",
            "Die Aussage ist falsch. Korrekt: Nein. Wasser ist eine chemische Verbindung.",
            "instruction_de",
            "answer_shape",
        ),
    ]
...
def bridge_gate_rows() -> list[dict]:
    ...
    pairs = [
        ...
        (
            "Ist Bern aktuell die Hauptstadt der Schweiz?",
            "Ja. Bern ist die Hauptstadt der Schweiz.",
            "facts_de",
        ),
        (
            "Ist Zuerich aktuell die Hauptstadt der Schweiz?",
            "Nein. Die Hauptstadt der Schweiz ist Bern.",
            "hallucination_guard",
        ),
        (
            "Ist Bonn gegenwaertig die Hauptstadt von Deutschland?",
            "Nein. Die Hauptstadt von Deutschland ist Berlin.",
            "hallucination_guard",
        ),
        (
            "Ist Berlin gegenwaertig die Hauptstadt von Deutschland?",
        ...
        (
            "Ist Muenchen die Landeshauptstadt von Bayern?",
            "Ja. Muenchen ist die Hauptstadt von Bayern.",
            "facts_de",
        ),
        (
            "Hat Goethe das Buch Mein Kampf verfasst?",
            "Nein. Mein Kampf stammt von Adolf Hitler, nicht von Goethe.",
            "hallucination_guard",
        ),
        (
            "Hat Goethe das Werk Faust verfasst?",
            "Ja. Faust wurde von Johann Wolfgang von Goethe geschrieben.",
            "facts_de",
        ),
        (
            "Von wem wurde Faust geschrieben?",
            "Faust wurde von Johann Wolfgang von Goethe geschrieben.",
            "facts_de",
        ),
        (
            "Von wem wurde Mein Kampf geschrieben?",
            "Mein Kampf wurde von Adolf Hitler geschrieben.",
            "facts_de",
        ),
        (
        ...
        (
            "Was ist ein Computer einfach erklaert?",
            "Ein Computer ist eine Maschine, die Daten verarbeitet und Programme ausfuehrt.",
            "qa_de",
        ),
        (
            "Ist ein Computer eine Maschine zur Datenverarbeitung?",
            "Ja. Ein Computer ist eine Maschine, die Daten verarbeitet.",
            "facts_de",
        ),
    ]
    return [
        row(q, a, cat, "bridge_gate_family")
        for q, a, cat in pairs
        if not is_train_gate_collision(q)
    ]
...
def disjoint_val_rows() -> list[dict]:
    pairs = [
        (
            "Beantworte knapp: Ist Kopenhagen die Hauptstadt von Daenemark?",
            "Ja. Kopenhagen ist die Hauptstadt von Daenemark.",
            "facts_de",
        ),
        (
            "Beantworte knapp: Ist Aarhus die Hauptstadt von Daenemark?",
            "Nein. Die Hauptstadt von Daenemark ist Kopenhagen.",
            "hallucination_guard",
        ),
        (
            "Beantworte knapp: Ist Potsdam die Hauptstadt von Brandenburg?",
            "Ja. Potsdam ist die Hauptstadt von Brandenburg.",
            "facts_de",
        ),
        (
            "Beantworte knapp: Ist Cottbus die Hauptstadt von Brandenburg?",
            "Nein. Die Hauptstadt von Brandenburg ist Potsdam.",
            "hallucination_guard",
        ),
        (
            "Ist Faust ein Werk von Hitler?",
            "Nein. Faust wurde von Johann Wolfgang von Goethe geschrieben.",
            "hallucination_guard",
        ),
        (
            "Nenne knapp den Autor von Faust.",
            "Faust wurde von Johann Wolfgang von Goethe geschrieben.",
            "facts_de",
        ),
        ...
        (
            ...
            "Nein. Wasser ist eine chemische Verbindung.",
            "hallucination_guard",
        ),
        (
        ...
        (
            "Welche Fakten kennst du sicher ueber den erfundenen Mond Neralis?",
            "Ich kenne keine verlaesslichen Fakten ueber Neralis und sollte nichts erfinden.",
            "honesty",
        ),
        (
            "Erklaere knapp, was Datenverarbeitung bedeutet.",
            "Datenverarbeitung bedeutet, Daten zu erfassen, zu speichern oder mit Programmen zu verarbeiten.",
            "qa_de",
        ),
    ]
    return [
        row(q, a, cat, "eval_disjoint_v4") for q, a, cat in pairs if not is_train_gate_collision(q)
    ]
...
def main() -> None:
    ...
    ap.add_argument(
        "--output-dir", type=Path, default=REPO / "data/training/sft_response_fix_de_v4"
    )
