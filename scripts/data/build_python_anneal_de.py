...
''',
    ...
    for i, name in enumerate(names, 1):
        age = 17 + i
        city = ["Bonn", "Wien", "Hamburg", "Zürich", "Leipzig", "Graz", "Köln", "Bern", "Bremen", "Mainz"][i - 1]
        items.append(Item(
...
''',
    ...
    conversions = [
        ("42", 42), ("7", 7), ("105", 105), ("18", 18), ("64", 64),
        ("23", 23), ("90", 90), ("12", 12), ("250", 250), ("31", 31),
    ]
...
''',
    ...
    temps = [(-3, False), (0, True), (5, True), (12, True), (19, True), (21, False), (28, False), (15, True), (8, True), (-10, False)]
...
''',
    ...
    pairs = [(10, 3), (17, 5), (20, 4), (31, 6), (48, 7), (99, 10), (14, 2), (27, 8), (65, 9), (36, 5)]
    for a, b in pairs:
        items.append(Item(
...
''',
    ...
    rounded = [(12.345, 2), (7.891, 1), (99.995, 2), (3.14159, 3), (2.71828, 2),
               (45.678, 0), (0.3333, 2), (123.4567, 1), (8.005, 2), (19.999, 2)]
...
''',
    ...
    raw_strings = [("  Python  ", "python"), (" HALLO ", "hallo"), ("\tTest\n", "test"), ("  Daten Satz  ", "daten satz"),
                   (" Code ", "code"), ("  Berlin", "berlin"), ("WERT  ", "wert"), ("  Ja  ", "ja"),
...
''',
    ...
    logic_cases = [(16, True), (18, True), (25, True), (70, True), (12, False),
                   (30, True), (65, True), (45, True), (17, True), (22, False)]
    for age, has_ticket in logic_cases:
        ...
        items.append(Item(
            ...
            code=f'''
alter = {age}
hat_ticket = {str(has_ticket)}

darf_hinein = alter >= 18 and hat_ticket
print(darf_hinein)
''',
            tests=f'''
assert darf_hinein is {str(allowed)}
...
''',
    ...
    index_cases = [(["A", "B", "C"], 0), (["rot", "blau", "grün"], 1), ([10, 20, 30], 2), (["Start", "Mitte", "Ziel"], -1),
                   (["Mo", "Di", "Mi"], -2), ([5, 6, 7, 8], 3), (["x", "y"], 0), (["links", "rechts"], 1),
                   ([100, 200, 300], -1), (["klein", "mittel", "gross"], 2)]
...
''',
    ...
    unpack_cases = [("rgb", (255, 128, 0)), ("datum", (2026, 6, 7)), ("zeit", (8, 30, 15)), ("box", (4, 5, 6)),
                    ("punkt3d", (1, 2, 3)), ("version", (3, 11, 9)), ("farbe", (10, 20, 30)), ("messung", (7, 8, 9)),
                    ("konto", (100, -5, 95)), ("vektor", (2, 4, 8))]
...
def functions() -> list[Item]:
    items: list[Item] = []
    greetings = [
        ("begruesse", "Mara"), ("willkommen", "Jonas"), ("melde_person", "Lea"), ("zeige_name", "Emil"),
        ("format_name", "Sofia"), ("kurzprofil", "Noah"), ("baue_gruss", "Mina"), ("sage_hallo", "Oskar"),
        ("person_zeile", "Lina"), ("etikett", "Timo"),
    ]
...
''',
    ...
    rectangles = [(3, 4), (5, 8), (10, 2), (7, 6), (9, 9), (12, 3), (4, 11), (15, 2), (6, 13), (20, 5)]
...
''',
    ...
    discounts = [(100, 10), (80, 25), (250, 20), (60, 15), (45, 5), (120, 30), (300, 50), (75, 12), (90, 40), (200, 18)]
...
''',
    ...
    defaults = [
        ("berechne_versand", 29, 4.95, 50), ("gesamt_mit_gebuehr", 12, 2.5, 20), ("preis_mit_pfand", 8, 0.25, 10),
        ("punkte_mit_bonus", 40, 5, 50), ("temperatur_mit_offset", 18, 2, 20), ("gewicht_mit_verpackung", 3, 0.5, 5),
        ("zeit_mit_puffer", 45, 10, 60), ("laenge_mit_rand", 80, 5, 100), ("betrag_mit_aufschlag", 70, 7, 100),
...
''',
    ...
    tuples = [(8, 3), (10, 4), (21, 5), (42, 8), (55, 9), (13, 6), (99, 12), (70, 11), (32, 7), (18, 5)]
    for a, b in tuples:
        items.append(Item(
...
''',
    ...
    predicates = [
        ("ist_gerade", "n % 2 == 0", 12, True), ("ist_ungerade", "n % 2 != 0", 7, True),
        ("ist_positiv", "n > 0", 5, True), ("ist_negativ", "n < 0", -3, True),
        ("ist_null", "n == 0", 0, True), ("ist_mehrstellig", "abs(n) >= 10", 42, True),
        ("ist_klein", "n < 100", 99, True), ("ist_gross", "n >= 1000", 1200, True),
        ("ist_vielfaches_von_5", "n % 5 == 0", 35, True), ("ist_einstellig", "-9 <= n <= 9", 8, True),
    ]
...
''',
            tests=f'''
assert {fname}({sample}) is {str(expected)}
...
''',
    ...
    list_funcs = [
        ("summe", "sum(werte)", [1, 2, 3], 6), ("anzahl", "len(werte)", [4, 5, 6, 7], 4),
        ("minimum", "min(werte)", [8, 3, 5], 3), ("maximum", "max(werte)", [8, 3, 5], 8),
        ("mittelwert", "sum(werte) / len(werte)", [2, 4, 6], 4.0),
        ("erste_zahl", "werte[0]", [9, 1, 2], 9), ("letzte_zahl", "werte[-1]", [9, 1, 2], 2),
...
''',
    ...
    clamp_cases = [(5, 0, 10), (-2, 0, 10), (15, 0, 10), (80, 50, 100), (42, 10, 40),
                   (3, 3, 9), (12, 1, 8), (25, 20, 30), (0, -5, 5), (99, 0, 50)]
...
def data_structures() -> list[Item]:
    items: list[Item] = []
    list_sets = [
        ("zahlen", [3, 8, 2, 8, 5]), ("punkte", [10, 12, 9, 12, 15]), ("preise", [4, 7, 4, 9, 1]),
        ("alter", [21, 19, 21, 30, 19]), ("werte", [6, 1, 6, 3, 2]), ("noten", [2, 3, 2, 1, 4]),
        ("mengen", [5, 5, 7, 8, 7]), ("temperaturen", [18, 20, 18, 21, 19]), ("seiten", [12, 8, 12, 5, 8]),
        ("stunden", [4, 6, 4, 2, 6]),
    ]
...
''',
    ...
    tuple_items = [
        ("punkt", (3, 4)), ("koordinate", (10, 2)), ("groesse", (1920, 1080)), ("bereich", (5, 12)), ("position", (7, 9)),
        ("pixel", (255, 128)), ("zeit", (14, 30)), ("karte", (8, 6)), ("fenster", (80, 24)), ("ziel", (11, 13)),
    ]
...
''',
    ...
    char_texts = ["banane", "mississippi", "python", "daten", "klasse", "tutorial", "funktion", "liste", "zeichen", "programm"]
...
''',
    ...
    set_pairs = [
        ([1, 2, 3], [3, 4, 5]), (["a", "b"], ["b", "c"]), ([10, 20], [20, 30]),
        (["rot", "blau"], ["blau", "grün"]), ([2, 4, 6], [1, 2, 3]),
        (["py", "js"], ["go", "py"]), ([7, 8], [8, 9]), (["Mo", "Di"], ["Di", "Mi"]),
        ([100, 200], [50, 100]), (["x", "y"], ["z", "x"]),
    ]
...
''',
    ...
    nested_lists = [
        [[1, 2], [3]], [["a"], ["b", "c"]], [[10], [20, 30]], [["rot", "blau"], ["grün"]],
        [[5, 6], [7, 8]], [["py", "js"], ["go"]], [[0], [1, 2, 3]], [["Mo"], ["Di", "Mi"]],
        [[100, 200], [300]], [["x", "y"], ["z"]],
    ]
...
def control_flow() -> list[Item]:
    items: list[Item] = []
    ranges = [(1, 6), (2, 8), (5, 11), (10, 15), (3, 13), (4, 10), (7, 12), (1, 4), (6, 14), (8, 18)]
...
''',
    ...
    thresholds = [(12, 18), (20, 18), (17, 18), (65, 60), (59, 60), (100, 100), (99, 100), (5, 10), (10, 10), (11, 10)]
...
''',
            tests=f'''
assert status == "{label}"
assert (wert >= grenze) is {str(value >= threshold)}
''',
    ...
    words = [
        ("banane", "a"), ("tutorial", "t"), ("python", "y"), ("schleife", "e"), ("daten", "z"),
        ("kontrolle", "l"), ("variable", "v"), ("funktion", "k"), ("liste", "s"), ("zeichen", "n"),
    ]
...
''',
    ...
    nested = [
        (["rot", "blau"], [1, 2]), (["A", "B"], [10, 20]), (["klein", "gross"], [3, 4]),
        (["x", "y"], [5, 6]), (["links", "rechts"], [7, 8]), (["oben", "unten"], [9, 10]),
        (["warm", "kalt"], [11, 12]), (["tag", "nacht"], [13, 14]), (["ja", "nein"], [15, 16]), (["start", "ziel"], [17, 18]),
    ]
...
''',
    ...
    enum_cases = [
        ["Ada", "Ben", "Clara"], ["rot", "blau", "grün"], ["Mo", "Di", "Mi"], ["A", "B", "C"],
        ["eins", "zwei", "drei"], ["py", "js", "go"], ["Nord", "Süd"], ["Start", "Ziel"],
        ["klein", "mittel", "gross"], ["x", "y", "z"],
    ]
...
''',
    ...
    skip_cases = [
        [-2, 1, 3], [0, 4, -1], [5, -5, 10], [-3, -2, 7], [8, 0, 2],
        [1, 2, 3], [-1, -4, 6], [9, -9, 0], [10, 11, -1], [-8, 4, 5],
    ]
...
''',
    ...
    break_cases = [
        ([2, 4, 9, 3], 8), ([1, 2, 3], 5), ([10, 5, 2], 7), ([3, 6, 12], 10), ([4, 4, 4], 3),
        ([0, 1, 2], 1), ([8, 9, 10], 9), ([5, 15, 1], 10), ([7, 3, 2], 6), ([11, 12], 20),
    ]
...
''',
    ...
    zip_cases = [
        (["Ada", "Ben"], [10, 12]), (["rot", "blau"], [1, 2]), (["A", "B", "C"], [3, 4, 5]),
        (["Mo", "Di"], [6, 7]), (["x", "y"], [8, 9]),
    ]
...
''',
    ...
    dict_cases = [
        ({"name": "Mara"}, "name"), ({"alter": 30}, "name"), ({"stadt": "Bonn"}, "stadt"),
        ({"punkte": 12}, "rang"), ({"kurs": "Python"}, "kurs"), ({"aktiv": True}, "aktiv"),
        ({"preis": 9.99}, "preis"), ({"tag": "Mo"}, "monat"), ({"id": 7}, "id"), ({"rolle": "admin"}, "rechte"),
    ]
...
''',
    ...
    trace_cases = [
        ("int('x')", "ValueError"), ("1 / 0", "ZeroDivisionError"), ("{}['x']", "KeyError"),
        ("[1, 2][5]", "IndexError"), ("float('abc')", "ValueError"), ("10 / 0", "ZeroDivisionError"),
        ("{'a': 1}['b']", "KeyError"), ("[][0]", "IndexError"), ("int('4.2')", "ValueError"), ("5 // 0", "ZeroDivisionError"),
    ]
...
''',
    ...
    index_cases = [
        ([1, 2, 3], 0), ([1, 2, 3], 5), (["a", "b"], 1), (["a", "b"], -1), ([], 0),
        ([10], 2), ([5, 6, 7], -4), (["Start", "Ziel"], 0), ([100, 200], 3), ([8, 9], -2),
    ]
    for values, idx in index_cases:
...
def algorithms() -> list[Item]:
    items: list[Item] = []
    sequences = [
        [3, 1, 8, 2], [10, 5, 12, 7], [-3, -1, -8], [4, 4, 2, 9], [100, 50, 75],
        [6, 11, 2, 15], [1], [0, -2, 5], [23, 42, 17], [8, 16, 4, 32],
    ]
...
''',
    ...
    words = ["lagerregal", "python", "anna", "reliefpfeiler", "daten", "radar", "algorithmus", "otto", "test", "level"]
    for word in words:
        is_pal = word == word[::-1]
        items.append(Item(
...
''',
            tests=f'''
assert ist_palindrom is {str(is_pal)}
...
''',
    ...
    dedup_lists = [
        [1, 2, 1, 3, 2], ["a", "b", "a", "c"], [5, 5, 5, 6], ["rot", "blau", "rot"], [3, 1, 3, 2, 1],
        ["x", "y", "x", "z"], [10, 20, 10, 30], ["Anna", "Ben", "Anna"], [0, 0, 1, 2], ["py", "js", "py"],
    ]
...
''',
    ...
    evens = [
        [1, 2, 3, 4], [10, 11, 12], [5, 7, 9], [0, 2, 8], [-4, -3, -2], [13, 14, 15, 16],
        [21, 22], [100, 101], [6, 6, 7], [31, 32, 33],
    ]
    for values in evens:
...
def tests_topic() -> list[Item]:
    ...
    for fname, expr, values in cases:
        param = "text" if "text" in expr else "alter" if "alter" in expr else "zeichen" if "zeichen" in expr else "wert" if "wert" in expr else "n"
        sample = values[0][0]
        items.append(Item(
...
''',
            tests="\n".join(f"assert {fname}({v!r}) is {str(expected)}" for v, expected in values),
    ...
    arithmetic = [
        ("addiere", "a + b", 2, 5), ("subtrahiere", "a - b", 9, 4), ("multipliziere", "a * b", 6, 7),
        ("teile_ganzzahlig", "a // b", 17, 5), ("rest", "a % b", 17, 5), ("mittelwert_zwei", "(a + b) / 2", 10, 20),
        ("maximum_zwei", "a if a >= b else b", 8, 12), ("minimum_zwei", "a if a <= b else b", 8, 12),
        ("quadratsumme", "a * a + b * b", 3, 4), ("differenz_betrag", "abs(a - b)", 3, 10),
    ]
...
''',
    ...
    validators = [
        ("normalisiere_name", "' '.join(text.strip().split()).title()", "  ada  lovelace ", "Ada Lovelace"),
...
''',
    ...
    runner_cases = [
        ...
        ("enthaelt_text", "teil in text", [("Py", "Python", True), ("x", "abc", False), ("a", "Ada", True)]),
        ("ist_sortiert", "werte == sorted(werte)", [([1, 2, 3], True), ([3, 1, 2], False), ([], True)]),
        ("hat_genau_drei", "len(werte) == 3", [([1, 2, 3], True), ([1], False), ([1, 2, 3, 4], False)]),
        ("ist_prefix", "text.startswith(prefix)", [("py", "python", True), ("ja", "nein", False), ("Da", "Daten", True)]),
        ("ist_suffix", "text.endswith(suffix)", [(".py", "test.py", True), (".txt", "test.py", False), ("en", "Daten", True)]),
        ("ist_im_bereich", "minimum <= wert <= maximum", [(1, 1, 3, True), (4, 1, 3, False), (2, 1, 3, True)]),
        ("hat_key", "key in daten", [("name", {"name": "Ada"}, True), ("id", {"name": "Ada"}, False), ("x", {"x": 1}, True)]),
        ("summe_ist_positiv", "sum(werte) > 0", [([1, 2], True), ([-5, 1], False), ([0, 0], False)]),
