...
def rows() -> list[dict]:
    bonn_current = [
        (
            "Ist Bonn momentan die Hauptstadt Deutschlands?",
            "Nein. Die Hauptstadt von Deutschland ist Berlin.",
        ),
        (
            "Welche Stadt ist heute die deutsche Hauptstadt, Bonn oder Berlin?",
            "Heute ist Berlin die Hauptstadt von Deutschland.",
        ),
        (
            "Darf man Bonn noch als Deutschlands Hauptstadt nennen?",
            "Nein. Bonn war frueher Hauptstadt; heute ist Berlin die Hauptstadt von Deutschland.",
        ),
        (
            "Ist Bonn aktuell Hauptstadt der Bundesrepublik?",
            "Nein. Aktuell ist Berlin die Hauptstadt Deutschlands.",
        ),
        (
            "Wenn jemand Bonn als deutsche Hauptstadt nennt, ist das heute korrekt?",
            "Nein. Heute ist Berlin die Hauptstadt von Deutschland.",
        ),
        (
            "Ist Bonn nur die ehemalige Hauptstadt Deutschlands?",
            "Ja. Bonn war frueher Hauptstadt; heute ist Berlin die Hauptstadt Deutschlands.",
        ),
        (
            "Welche Rolle hat Bonn im Vergleich zu Berlin?",
            "Bonn war frueher Hauptstadt. Heute ist Berlin die Hauptstadt Deutschlands.",
        ),
        (
            "Ist Berlin oder Bonn die aktuelle Hauptstadt von Deutschland?",
            "Berlin ist die aktuelle Hauptstadt von Deutschland.",
        ),
    ]
    photosynthesis = [
        (
            "Erklaere Photosynthese in einem Satz.",
        ...
        (
            "Wie gewinnen Pflanzen bei Photosynthese Energie?",
            "Pflanzen nutzen Licht, um aus Wasser und Kohlendioxid Zucker herzustellen; dabei entsteht Sauerstoff.",
        ),
        (
            "Was machen Pflanzen bei der Photosynthese?",
            "Pflanzen wandeln mit Licht Wasser und Kohlendioxid in Zucker um und geben Sauerstoff ab.",
        ),
        (
            "Welche Stoffe entstehen bei Photosynthese?",
            "Bei der Photosynthese bilden Pflanzen Zucker und Sauerstoff.",
        ),
        (
            "Was ist Photosynthese einfach erklaert?",
            "Photosynthese bedeutet: Pflanzen nutzen Licht, um Zucker zu bilden, und setzen Sauerstoff frei.",
        ),
        (
            "Warum ist Licht fuer Photosynthese wichtig?",
            "Licht liefert die Energie, mit der Pflanzen Zucker herstellen und Sauerstoff freisetzen.",
        ),
        (
            "Welche Lebewesen betreiben typischerweise Photosynthese?",
            "Vor allem Pflanzen, Algen und manche Bakterien betreiben Photosynthese.",
        ),
        (
            "Stimmt es, dass bei Photosynthese Sauerstoff entsteht?",
            "Ja. Pflanzen bilden bei der Photosynthese Zucker und setzen Sauerstoff frei.",
        ),
    ]
    keepalive = [
        (
            "Ist Hamburg aktuell die Hauptstadt von Bayern?",
    ...
    honesty = [
        (
            "Welche Farbe hatte der erfundene Planet Zelvorn?",
            "Dazu habe ich keine verlaessliche Information. Zelvorn klingt erfunden oder unklar.",
        ),
        (
            "Kennst du sichere Fakten ueber den Fantasieort Neralon?",
            "Nein. Dazu habe ich keine verlaesslichen Fakten und sollte nichts erfinden.",
        ),
        (
            "Wie antwortest du, wenn eine Frage erfundene Details enthaelt?",
    ...
    out: list[dict] = []
    out.extend(
        row(q, a, "hallucination_guard", "bonn_current_repair", "bonn_current")
        for q, a in bonn_current
    )
    out.extend(
        row(q, a, "qa_de", "photosynthesis_repair", "photosynthesis") for q, a in photosynthesis
    )
    out.extend(
    ...
    out.extend(
        row(q, a, "facts_de", "facts_keepalive", "facts_keepalive") for q, a in keepalive[3:6]
    )
...
def main() -> None:
    ...
    ap.add_argument(
        "--output-dir",
        type=Path,
        default=REPO / "data/training/sft_response_fix_de_v7_bonn_photo_patch",
    )
