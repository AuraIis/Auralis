...
BASE_CKPT="${BASE_CKPT:-checkpoints/sft_response_fix_de_v4_anchor220_polarity_patch_80/sft_smoke_step_80.pt}"
...
TRAIN="${TRAIN:-data/training/sft_response_fix_de_v5/core_train.helix.jsonl}"
VAL="${VAL:-data/training/sft_response_fix_de_v5/val.helix.jsonl}"
RESULT_ROOT="${RESULT_ROOT:-data/eval/checkpoint_tests}"
REPORT_DIR="${REPORT_DIR:-reports}"

mkdir -p "$RESULT_ROOT" "$REPORT_DIR"

for steps in ${STEPS:-60 120 180}; do
  tag="sft_response_fix_de_v5_from_v4best_${steps}"
  ...
  python scripts/sft/smoke_sft_de.py \
    ...
    --warmup-steps 30 \
    --eval-every "$steps" \
    --category-weights "${CATEGORY_WEIGHTS:-facts_de=1.5,hallucination_guard=0.85,qa_de=1.3,honesty=1.8,instruction_de=1.2}" \
    --eos-loss-weight 10 \
    --diag-json "$diag" \
    --save-final
  ...
  for gate in sft_response_fix_chat_gate_v2 sft_response_fix_chat_gate_v3_holdout; do
...
for path in sorted(Path("reports").glob("semantic_gate_sft_response_fix_de_v5_from_v4best_*_2026-05-28.json")):
