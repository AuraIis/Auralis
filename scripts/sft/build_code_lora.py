...
import sys, json, random, argparse, pathlib
REPO = pathlib.Path("/workspace/v2data")
sys.path.insert(0, str(pathlib.Path(__file__).resolve().parent.parent / "data"))
from code_format import tab_indent_fenced  # pretrain code is tab-indented; normalize SFT code blocks to match
RES_OPEN = "<result>"
...
def main():
    ...
    ap.add_argument("--code", default=str(REPO / "data/training/code_verified_v1/verified_code.jsonl"))
    ...
    rows += [{"text": tab_indent_fenced(r["text"]), "source": "code_verified"} for r in code] * a.code_rep
    chat = [r for r in read(a.chat)
            if "<tool:" not in r.get("text", "") and RES_OPEN not in r.get("text", "")
