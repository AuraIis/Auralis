...
import os, sys, argparse, pathlib, torch
...
def generate(model, sp, prompt, device, max_new=90, temperature=0.0, top_k=0, rep_pen=1.3):
    ...
    ids=sp.EncodeAsIds(prompt); inp=torch.tensor([ids],device=device); gen=[]
    for _ in range(max_new):
        ...
        if nxt==end_id: break
        gen.append(nxt); inp=torch.cat([inp,torch.tensor([[nxt]],device=device)],1)
