...
import gzip, json, sys, urllib.request, pathlib
...
def download():
    ...
    if GZ.exists() and GZ.stat().st_size>1_000_000:
        print("schon vorhanden:", GZ, GZ.stat().st_size, "bytes"); return
...
def best_reply(msg):
    cands=[m for m in msg.get("replies",[]) if m.get("role")=="assistant" and not m.get("deleted")]
...
def main():
    ...
    with gzip.open(GZ,"rt",encoding="utf-8") as f:
        for line in f:
            ...
            t=json.loads(line); n+=1
            prompt=t.get("prompt") or t
            if prompt.get("role")!="prompter" or prompt.get("deleted"): continue
            lang=prompt.get("lang")
            if lang not in ("de","en"): continue
            a=best_reply(prompt)
            if not a: continue
            u=(prompt.get("text") or "").strip(); ans=(a.get("text") or "").strip()
            if not (10<=len(u)<=1500 and 10<=len(ans)<=4000): continue
            row={"text":helix(u,ans),"category":f"oasst_{lang}","question":u,"source":"oasst1"}
    ...
    if de: print("DE-Beispiel:", de[0]["question"][:90])
