...
"""
import os, sys, re, json, argparse, pathlib, urllib.request, tempfile, subprocess
...
DENY = ["import os", "import sys", "import subprocess", "import socket", "import shutil",
        ...
        "os.", "sys.", "subprocess", "pathlib", "shutil", "remove(", "rmdir", "system(",
        "popen", "fork", "while True", "while 1"]
...
TOPICS = ["Zahlen/Arithmetik", "Strings/Text", "Listen/Sortieren", "Schleifen/Zaehlen",
          "Rekursion", "Mathe (ggT, Fakultaet, Fibonacci)", "Suchen/Filtern", "Dictionaries/Zaehlen"]
...
def gen_phase(a, raw_path):
    ...
    while len(seen) < a.gen_target and call < a.max_calls:
        ...
        for line in txt.splitlines():
            ...
            if not q or q in seen or not r.get("code") or not r.get("funktion") or not r.get("tests"):
                continue
            seen.add(q); r["_topic"] = topic
...
def verify_phase(a, raw_path, out_path):
    ...
    for r in raw:
        ...
        answer = f"```python\n{code.strip()}\n```" + (f"\n\n{erkl}" if erkl else "")
        fout.write(json.dumps({"text": helix(q, answer), "source": "code_verified", "has_tool": False,
                               "meta": {"q": q, "funktion": fname}}, ensure_ascii=False) + "\n")
    ...
    for k in ["seen", "safe", "compile", "pass_", "kept"]:
        print(f"  {k:9} {st[k]}")
    if st["seen"]:
        print(f"  compile-rate {st['compile']/st['seen']:.0%} | pass-rate {st['pass_']/max(1,st['compile']):.0%} | overall kept {st['kept']/st['seen']:.0%}")
...
def main():
    ...
    print(f"=== verified-code | teacher={a.teacher} phase={a.phase} target={a.target} gen_target={a.gen_target} ===", flush=True)
