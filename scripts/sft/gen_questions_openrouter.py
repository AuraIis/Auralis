...
TOPICS_DE = [
    ...
    "persoenliche Finanzen (allgemein)",
    "Bildung und Lernen",
...
def cfg():
    return {
        ...
        "de_reasoning": dict(
            ...
            n=1200,
            per=12,
            topics=["Logik", "Mathe im Alltag", "Vergleiche", "Schlussfolgern", "Zeit/Rechnen"],
        ...
        "de_definition": dict(
            ...
            n=800,
            per=10,
            topics=["Superlative", "Vergleiche", "Rankings", "mehrdeutige Begriffe"],
...
def build_prompt(c, topic):
    ...
    return (
        f'Erzeuge {c["per"]} {lang} Nutzer-Fragen {langword} zum Bereich "{topic}": {c["instr"]}.\n'
...
def extract(text):
    ...
    for ln in text.splitlines():
        ...
        ln = ln.strip(' "')
        if (
            (8 <= len(ln) <= 240 and ln.endswith(("?", ".", ":")))
            or (8 <= len(ln) <= 240 and "?" in ln)
            or (8 <= len(ln) <= 240 and ln)
        ):
            out.append(ln)
    return out
...
def main():
    ...
    C = cfg()
    if args.only:
        C = {k: v for k, v in C.items() if k in args.only.split(",")}
    ...
    def work(cat, c, topic):
        if catcount[cat] >= c["target"]:
            return
        try:
            txt = call(args.model, build_prompt(c, topic), key)
        except Exception as e:
            print("FEHLER", cat, repr(e)[:100], file=sys.stderr)
            return
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.workers) as ex:
        ...
        for n, _ in enumerate(concurrent.futures.as_completed(futs), 1):
            if n % 50 == 0:
                print(f"  calls {n}/{len(jobs)} | fragen {len(rows)}", file=sys.stderr)
    ...
    for k, v in Counter(c for c, _ in rows).most_common():
        print(f"  {k}: {v}")
