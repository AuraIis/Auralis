...
import os, sys, re, json, random, argparse, pathlib

REPO = pathlib.Path("/workspace/v2data")
sys.path.insert(0, str(REPO / "scripts/sft")); sys.path.insert(0, str(REPO))
...
def g_sub(r):
    a = r.randint(50, 999); b = r.randint(7, a - 1)
    q = r.choice([f"Was ist {a} minus {b}?", f"Berechne {a} - {b}.", f"Wie viel ist {a} weniger {b}?"])
...
def g_div(r):
    b = r.randint(2, 25); k = r.randint(2, 40); a = b * k
    q = r.choice([f"Was ist {a} geteilt durch {b}?", f"Berechne {a} ÷ {b}.", f"Wie viel ist {a} / {b}?"])
    return q, f"{a} / {b}", r.choice([f"{a} geteilt durch {b} ergibt {{res}}.", "Das ergibt {res}."])

def g_percent(r):
    p = r.choice([5, 10, 12, 15, 20, 25, 30, 40, 50, 75]); n = r.randint(2, 40) * 20
    q = r.choice([f"Was sind {p}% von {n}?", f"Berechne {p} Prozent von {n}.", f"Wie viel sind {p}% von {n}?"])
    return q, f"{n} * {p} / 100", r.choice([f"{p}% von {n} sind {{res}}.", "Das sind {res}."])

def g_hours_min(r):
    h = r.randint(1, 9); m = r.randint(1, 59)
...
def g_recipe_scale(r):
    per = r.choice([2, 3, 4]); g = r.randint(2, 20) * 10; t = r.randint(2, 8)
    q = f"Ein Rezept fuer {per} Personen braucht {g} Gramm Mehl. Wie viel fuer {t} Personen?"
    return q, f"{g} * {t} / {per}", "Man braucht {res} Gramm."

BASE_GENS = [(g_add, 3), (g_sub, 3), (g_mul, 4), (g_div, 3),
        (g_percent, 8), (g_discount_price, 8), (g_discount_amount, 6), (g_markup_price, 6),
        (g_hours_min, 6), (g_days_hours, 4), (g_min_sec, 4), (g_km_m, 3),
        (g_speed_dist, 7), (g_dist_time, 6),
        (g_word_total, 6), (g_word_change, 6), (g_price_total, 6), (g_recipe_scale, 6),
        (g_fraction, 5), (g_avg, 5), (g_square, 3), (g_sqrt, 3),
        (g_en_mul, 3), (g_en_pct, 4)]


PHASE2_SIMPLE_REBUMP_GENS = [(g_add, 6), (g_sub, 6), (g_mul, 7), (g_div, 6),
        (g_percent, 8), (g_discount_price, 8), (g_discount_amount, 6), (g_markup_price, 6),
        (g_hours_min, 6), (g_days_hours, 4), (g_min_sec, 4), (g_km_m, 3),
        (g_speed_dist, 7), (g_dist_time, 6),
        (g_word_total, 6), (g_word_change, 6), (g_price_total, 6), (g_recipe_scale, 6),
        (g_fraction, 5), (g_avg, 5), (g_square, 6), (g_sqrt, 6),
        (g_en_mul, 3), (g_en_pct, 4)]
...
def sample_qa(rng, n, path):
    p = pathlib.Path(path)
    if not p.exists():
        print(f"WARN no QA source at {path}", file=sys.stderr); return []
    cand = []
...
def main():
    ...
    out = pathlib.Path(a.out_dir + ("_" + a.mode)); out.mkdir(parents=True, exist_ok=True)
