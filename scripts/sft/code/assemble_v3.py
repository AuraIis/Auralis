...
random.seed(50)
REPO = "/workspace/v2data"; D = REPO + "/data/training"; OUT = D + "/sft_code_v3"; os.makedirs(OUT, exist_ok=True)
...
ver = jl(D + "/code_verified_v1/verified_code.jsonl")  # filtered set is already rendered (text field)
for _ in range(2):
    for r in ver:
...
for r in corr[:900]: rows.append({"text": r["text"], "src": "corrective"})
ab = jl(D + "/calib_verified_v1/verified_calib.jsonl"); random.shuffle(ab)
for r in ab[:100]: rows.append({"text": r["text"], "src": "abstain"})
