...
SYS = (
    "Du erstellst deutsche Mathe-Textaufgaben mit Schritt-fuer-Schritt-Loesung fuer KI-Training.\n"
...
TOPICS = [
    ...
    "Zeit und Alter",
    "Mengen aufteilen und verteilen",
    "einfache Geometrie (Flaeche, Umfang)",
    "Mischungen und Verhaeltnisse",
    "Durchschnitt berechnen",
    "Bruchrechnung im Alltag",
    "einfache Logik und Schlussfolgern",
    "Zinsen einfach",
    "Einheiten umrechnen",
    "Rabatt und Aufschlag",
    "Anzahl und Stueckzahl",
]
...
def main():
    ...
    def work(topic):
        nonlocal done
        prompt = (
            f'Erzeuge {args.per} deutsche Mathe-Textaufgaben zum Bereich "{topic}" mit Schritt-fuer-Schritt-Loesung. '
        ...
        try:
            txt = call(args.model, prompt, key)
        except Exception as e:
            print("FEHLER", topic, repr(e)[:90], file=sys.stderr)
            return
        for q, a in parse(txt):
            k = norm(q)
            with lock:
                if len(rows) >= args.target or k in seen:
                    continue
                seen.add(k)
                rows.append(
                    {
                        "text": helix(q, a),
                        "category": "reasoning_math_de",
                        "question": q,
                        "source": "gen_de",
                    }
                )
        with lock:
            ...
            if done % 25 == 0:
                print(f"  calls {done}/{len(jobs)} | examples {len(rows)}", file=sys.stderr)
