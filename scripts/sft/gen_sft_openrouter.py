...
def main():
    ...
    ap.add_argument(
        "--questions",
        default=None,
        help="optional file: 'category<TAB>question' or one question per line",
    )
    ...
    def work(i, cat, q):
        ...
        try:
            ...
            print(f"FEHLER [{cat}] {q}: {repr(e)[:160]}", file=sys.stderr)
            return
        text = (
        ...
        if d % 25 == 0:
            print(f"  {d}/{len(seeds)} ({tot} tok)", file=sys.stderr)
