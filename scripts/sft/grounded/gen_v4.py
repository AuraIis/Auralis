...
rows = [json.loads(l) for l in open(OUT + "/grounded_v3.jsonl", encoding="utf-8")]  # inherit all of v3

# ---------- (A) dense multi-sentence prose (Greifenau-style, fictional towns) ----------
TOWNS = ["Talheim","Moosbach","Eichfeld","Rauental","Sonnberg","Weidach","Kirchsee","Hagenbruck",
         "Falkenried","Brunnthal","Erlbach","Auental","Lichtenfeld","Wieshofen","Steinau",
         "Reichenau","Hochdorf","Lindenberg","Buchholz","Neubrunn"]
THINGS = ["ihre Toepferkunst","ihren Wochenmarkt","ihre alte Muehle","ihr Weinfest","ihre Glasblaeserei",
          "ihren Kraeutergarten","ihre Holzbruecke","ihr Heimatmuseum","ihre Kaesereien","ihr Lichterfest"]
LAND = ["ein Brunnen aus Stein","eine alte Kirche","ein hoher Uhrturm","ein kleines Rathaus","eine steinerne Bruecke"]
for t in TOWNS:
    year = ri(1100, 1850); pop = ri(800, 9000); thing = ch(THINGS); land = ch(LAND)
    ctx = (f"Die Kleinstadt {t} liegt in einem Tal. Sie wurde {year} gegruendet. "
    ...
    rows.append(it(ctx, ch([f"Wie heisst der Buergermeister von {t}?",
                            f"Welche Sprache spricht man in {t}?",
                            f"Wie hoch liegt {t} ueber dem Meer?"]), ch(REF), False))

# ---------- (B) count: named-but-uncounted -> refuse ; counted -> answer ----------
CONT = [("Korb","liegen",["Aepfel","Birnen","Bananen","Kirschen","Pflaumen"]),
...
for _ in range(55):  # uncounted -> refuse
    c, v, pool = ch(CONT); pick = random.sample(pool, 3)
    lst = ", ".join(pick[:-1]) + " und " + pick[-1]
    ctx = f"Im {c} {v} {lst}."
    rows.append(it(ctx, f"Wie viele {ch(pick)} {v.replace('sind','sind').replace('liegen','liegen')} im {c}?".replace("  ", " "),
                   ch(REF), False))
for _ in range(45):  # counted -> answer
    c, v, pool = ch(CONT); a, b = random.sample(pool, 2); na = ri(2, 9); nb = ri(2, 9)
...
for _ in range(28):
    e = ch(EVENTS); a = ri(8, 14); b = ri(a + 2, 21)
...
for _ in range(28):
    p = ch(PLACES); a = ri(6, 11); b = ri(a + 4, 22)
...
print(f"grounded_v4: {len(uniq)} | answerable {na} ({100*na//len(uniq)}%) | refuse {len(uniq)-na}")
