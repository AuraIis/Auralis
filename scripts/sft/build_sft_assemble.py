...
EVAL = [
    ...
    "Erkläre kurz, was Wasser ist.",
    "Schreibe einen einfachen deutschen Satz über Wasser.",
    ...
    "Nenne drei Bundesländer in Deutschland.",
    "Was ist 12 plus 15?",
    "Warum muss man Zähne putzen?",
    "Wer hat die Glühbirne erfunden",
    "Was ist die Hauptstadt von Italien?",
    "Wo liegt Deutschland?",
    "Welches ist das bevölkerungsreichste Land Europas?",
]
...
def main():
    ...
    for path, src, cap in SOURCES:
        ...
        for line in open(p, encoding="utf-8"):
            ...
            if un in seen:
                ...
                continue
            seen.add(un)
            rows.append({"text": render(u, a), "source": src})
            per[src] += 1
            n += 1
            if cap and n >= cap:
                break
    ...
    (OUT / "assemble_manifest.json").write_text(
        json.dumps(
            {
                "train": len(train),
                "val": len(val),
