...
def main():
    ...
    ap.add_argument(
        "--sample-rate", type=float, default=0.3, help="verify fraction of non-critical rows"
    )
    ap.add_argument(
        "--always",
        default="de_factual,de_current,de_correction,de_definition,en_factual",
    ...
    kept = list(skipped)
    stats = Counter()
    stats["accept(skipped)"] = len(skipped)
    tot = 0
    parsefail = 0
    done = 0
    lock = threading.Lock()
    ...
    def process(r):
        ...
        ans = ans_of(r["text"])
        try:
            raw, us = call(args.model, q, ans, key)
        except Exception:
            with lock:
                stats["error"] += 1
                kept.append(r)
        ...
        v = parse_json(raw)
        with lock:
            tot += us.get("total_tokens", 0)
            done += 1
            d = done
            if not v:
                parsefail += 1
                stats["accept(parsefail)"] += 1
                kept.append(r)
            elif v.get("verdict") == "reject" and str(v.get("fixed", "")).strip():
                r2 = dict(r)
                r2["text"] = wrap(q, str(v["fixed"]).strip())
                r2["corrected"] = True
                r2["rule"] = v.get("rule")
                kept.append(r2)
                stats[f"reject->fixed(rule{v.get('rule')})"] += 1
                ...
                kept.append(r)
        if d % 50 == 0:
            print(f"  verify {d}/{len(to_verify)} ({tot} tok)", file=sys.stderr)
