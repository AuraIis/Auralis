...
def _greedy_generate(
    ...
    model.eval()
    with torch.no_grad():
        if device.type == "cuda" and hasattr(model, "generate"):
            # O(1)-per-token incremental decode (token-identical to the
            # full re-forward loop; verified by scripts/dev/bench_infer.py).
            new_tokens = model.generate(x, max_new_tokens=max_new_tokens)[0].tolist()
        else:
            new_tokens = []
